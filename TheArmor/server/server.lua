...
local Kicks = {}
local Warns = {}
-- Gang Leaks ZomerX#6153 discord = discord.gg/5xHA3RY3Fq
...
[+]         db                  mm     db             `7MM                         mm    
[+]        ;MM:                 MM                      MM                         MM    
[+]       ,V^MM.    `7MMpMMMb.mmMMmm `7MM       ,p6"bo  MMpMMMb.  .gP"Ya   ,6"Yb.mmMMmm  
[+]      ,M  `MM      MM    MM  MM     MM      6M'  OO  MM    MM ,M'   Yb 8)   MM  MM    
[+]      AbmmmqMA     MM    MM  MM     MM      8M       MM    MM 8M""""""  ,pm9MM  MM    
[+]     A'     VML    MM    MM  MM     MM      YM.    , MM    MM YM.    , 8M   MM  MM    
[+]   .AMA.   .AMMA..JMML  JMML.`Mbmo.JMML.     YMbmd'.JMML  JMML.`Mbmmd' `Moo9^Yo.`Mbmo                           
   
]])  
                                                                                
      print("^"..RandomColor.."[+] ["..Config.ServerConfig.ServerName.." The Armor AntiCheat System] : The Armor AntiCheat System Created by 7krh#0490 | Russia#9715 ^0")
      ...
      PerformHttpRequest("http://"..Config.ServerConfig.ServerIp.."/info.json", function(ERROR, DATA, RESULT)
          if DATA ~= nil then
              ...
              print("^"..RandomColor.."[+] ["..Config.ServerConfig.ServerName.." The Armor AntiCheat System] : Server Build : "..string.gsub(string.gsub(string.gsub(json.server, "FXServer", ""), "win32", ""), "-master SERVER v1.0.0.", "").."".."^0")
          end
      end)
      PerformHttpRequest("http://"..Config.ServerConfig.ServerIp.."/info.json", function(ERROR, DATA, RESULT)
        if DATA ~= nil then
...
AddEventHandler("playerDropped", function(REASON)
    ...
    print("^"..RandomColor.."[+] ["..Config.ServerConfig.ServerName.." The Armor AntiCheat System] : Player "..GetPlayerName(SRC).." Disconnected From The Server  |  Reason : ("..REASON..")^0")
...
AddEventHandler("playerConnecting", function (name, setKickReason, deferrals)
    local SRC      = source
    local BANFILE = LoadResourceFile(GetCurrentResourceName(), "BanList/Bans.json")
    print("^"..RandomColor.."[+] ["..Config.ServerConfig.ServerName.." The Armor AntiCheat System] : Player "..name.." Connecting ...^0")
                ...
                if tonumber(SRC) ~= nil then
                    for k,v in pairs(TABLE) do
                        if v.Steam == QBCore.Functions.GetIdentifier(SRC, 'steam') or v.License == QBCore.Functions.GetIdentifier(SRC, 'license') or v.Discord == QBCore.Functions.GetIdentifier(SRC, 'discord') or v.Token[0] == GetPlayerToken(SRC, 0) or v.Token[1] == GetPlayerToken(SRC, 1) or v.Token[2] == GetPlayerToken(SRC, 2) or v.Token[3] == GetPlayerToken(SRC, 3) or v.Token[4] == GetPlayerToken(SRC, 4) then
                            ...
                            CancelEvent()        
                            print("^"..RandomColor.."[+] ["..Config.ServerConfig.ServerName.." The Armor AntiCheat System] : Ban Player Try To Join "..name.."^0")
